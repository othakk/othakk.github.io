...
function ProjectImage({ src, alt }: { src: string; alt: string }) {
  ...
  if (error || !src) {
    return (
      <div className="w-10 h-10 md:w-11 md:h-11 rounded-lg bg-primary/10 flex items-center justify-center text-lg font-bold text-primary shrink-0 mt-1 md:mt-0">
        {alt.charAt(0)}
      </div>
    );
  }
